...
class Payment implements Money {
    ...
    public boolean payFees(String vehicleId, int fees) {
        String card = getCard();
        System.out.println("Fee of " + fees + " paid by " + card);
        return true;
    }

    public boolean payFees(String vehicleId, int fees, int elecFees) {
        String card = getCard();
        System.out.println("Fee of " + (fees + elecFees) + " paid by " + card);
        return true;
    }
}
...
class ParkingLot {
    ...
    class Floor {
        ...
        class Lot { //Struct
            ...
            //Lot constructor to specify the type of lot
        ...
        Floor(int[] lotList) {
            ...
            for (int j = 0; j < 5; j++) {
                for (int i = 0; i < lotList[j]; i++) {
                    this.lotList.add(new Lot(j));
                }
            }
        }
                    ...
                    if (lot.vehicleID.equals(leaving.substring(1))) {
                        ...
                        while (!paid) {
                            ...
                            switch (paymentMethod) {
                                case 1:
                                    System.out.println("Please pay the fees of Rs. " + fees + " at the exit.");
                                    paid = true;
                                    break;
                                case 2:
                                    fee.payFees(leaving.substring(1), fees);
                                    paid = true;
                                    break;
                                default:
